...
% LegPosition = [linspace(0,t,tDiv)', [-75*ones(tInitial,1); linspace(-75,-20,tJump)'], [-75*ones(tInitial,1); linspace(-75,-20,tJump)']];
...
% [-75*ones(waitTimeSteps,1); linspace(-75,-20,takeOffTimeSteps)'],...
% [-75*ones(waitTimeSteps,1); linspace(-75,-20,takeOffTimeSteps)']]; 

LegPosition = [linspace(0,totalTime,totalTimeSteps)',...
    [-45*ones(waitTimeSteps,1); linspace(-45,-10,takeOffTimeSteps)'],...
    [-45*ones(waitTimeSteps,1); linspace(-45,-10,takeOffTimeSteps)']]; 
...
t = linspace(0,totalTime,totalTimeSteps);
size(t)
...
xi = SpringMatrix(:,1);
xf = SpringMatrix(:,2);
x = zeros(size(SpringMatrix,1),200);
for c = 1:size(SpringMatrix,1)
    x(c,:) = linspace(xi(c),xf(c),200);
end
...
LegTorque = zeros(size(linkLength,2),size(x,2),size(SpringMatrix,1));
    ...
    for j = 1:size(linkLength,2)
        for l = 1:size(x,2)
            if linkLength(j)*2 > xf(i)
                ...
                LegTorque(j,l,i) = (x(i,l) - xi(i))*k(i)*sqrt(linkLength(j)^2 - (x(i,l)/2)^2);
...
%% Display Springs
num = 5;
LegTorque = [t', LegTorque(link(num),:,spring(num))']
% LegTorque = [t', zeros(totalTimeSteps,1)];
size(LegTorque)