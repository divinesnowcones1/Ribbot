...
SpringMatrix = [
%     3*in2m, 5.94*in2m, 2.2*lbf2N;... %9044K193
...
xi = SpringMatrix(:,1);
xf = SpringMatrix(:,2) * 0.99;             %Max spring displacement will be 99% of maximum rating
...
tempAngle = 0;
tempFyavg = 0;
minInitialForce = m*g*1.2;
maxInitialForce = m*g*1.3;
for i = 1:size(Fyavg,3)
    for j = 1:size(linkLength,2)
        tempAngle = AngleMin(j,1,i);
        tempFyavg = Fyavg(j,1,i);
        if Fyavg(j,1,i) > mean(Freq) && min(Fy(j,length(x),i)) < maxInitialForce && min(Fy(j,length(x),i)) > minInitialForce ...
...
clc;
 for i = 1:length(spring)
    ...
    num = i;
    k = SpringMatrix(spring(num),3);
    Fx = k*(x(spring(num),:)-xi(spring(num)));
    a = linkLength(link(num));
    y = sqrt(a^2 - xf(spring(num))^2/4) + sqrt(a^2 - xf(spring(num))^2/4);
    Flock = k*y*(1-a/sqrt(4*a^2 - y^2));
    ...
    disp(['Link Length: ',num2str(linkLength(link(num))/in2m),' in']);
    disp(['Max Force on Link: ' num2str(max(Fx)),'']);
    disp(['Max Force odn Lock: ' num2str(Flock),'']);
    disp('----');
% x = 11.5;
% a = 6.5;
% k = 0.62;
% y = sqrt(a^2 - x^2/4) + sqrt(a^2 - x^2/4);
% Fx = k*11.5;
% Fy = k*y*(1-6.5/sqrt(4*a^2 - y^2))
end
    figure(num);
    ...
    figure(num+2);
    plot(rad2deg(angle(link(num),:,spring(num))),Fx);
    xlabel('Theta');
    ylabel('Fx (N)');
    title('Force on Spring vs Theta');
